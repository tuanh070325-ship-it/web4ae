...
| `analytics_events` | Raw page, product, search, cart, checkout, and order analytics events | FK to users/products/orders when present; indexed by date, event, session, path, product |
| `analytics_daily_stats` | Optional daily aggregate counters for dashboard-style analytics | `date_key` primary key |
| `analytics_product_stats` | Optional product-level aggregate counters | FK `product_id -> products.id` |
...
## Deterministic Admin Demo Dataset

For local development after schema sync, run:

```powershell
npm --prefix shopanime_be run db:seed:demo
```

This command is intentionally separate from the one-time `mockData.ts` seed. It keeps catalog data but resets operational/demo tables, then builds a coherent timeline:

- customer accounts and default addresses
- carts and wishlists
- 30 days of orders and order items
- product stock and sold counts recalculated from non-cancelled order items
- analytics events whose session dates, product ids, order ids, and revenue match the generated orders

It is development-only and should not be run against production data.

## Analytics Tables

`analytics_events` stores the raw event stream. Important fields:

- `event_name`: stable event name such as `page_view`, `product_click`, `add_to_cart`, or `order_completed`
- `event_type`: traffic, product, engagement, or commerce
- `session_id`: anonymous browser session id generated by the frontend
- `user_id`, `product_id`, `order_id`: nullable foreign keys for attribution
- `path`, `referrer`, `utm_source`, `utm_medium`, `utm_campaign`: traffic attribution
- `device_type`, `browser`, `user_agent`: basic client hints
- `ip_hash`: salted hash, never raw IP
- `metadata`: whitelisted JSON metadata such as source, position, query, quantity, price, and revenue

`analytics_daily_stats` and `analytics_product_stats` are aggregate tables for faster future reads. Current admin analytics queries still read raw events so reports remain correct even if aggregate backfills are skipped.
