      ...
      <div className="max-w-[1000px] mx-auto relative z-10">
        ...
        {orders.length > 0 && (
          <div className="bg-[#111216] rounded-xl overflow-hidden shadow-2xl border border-[#2e333d]">
            <div className="grid grid-cols-5 px-8 py-5 bg-[#0a0a0c] border-b border-[#2e333d] font-bold text-white text-sm">
              <div>ORDER ID</div>
              <div>DATE</div>
              <div>TOTAL PRICE</div>
              <div>STATUS</div>
            ...
            <div className="divide-y divide-[#24262f]">
              {orders.map((order, i) => (
                <div key={order.id} className={`grid grid-cols-5 items-center px-8 py-5 ${i % 2 !== 0 ? 'bg-[#1a1b22]' : 'bg-[#111216]'}`}>
                  <div className="text-white font-medium">#{order.order_code || order.id}</div>
                  <div>{order.created_at ? new Date(order.created_at).toLocaleDateString() : "N/A"}</div>
                  <div>{formatUsd(order.total_amount ?? order.total)}</div>
                  <div>
                  ...
                  <div className="flex justify-end gap-3">
                    <button className="px-4 py-1.5 border border-[#e63946] text-[#e63946] hover:bg-[#e63946]/10 rounded font-bold text-xs tracking-wider transition-colors">
                      VIEW DETAILS
                    </button>
                  </div>
                </div>
